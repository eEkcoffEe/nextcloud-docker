...
### Расположение данных пользователей

Файлы пользователей хранятся в директории `/srv/mergerfs/pool1/nextcloud` на хост-системе. Эта директория монтируется в контейнер Nextcloud по пути `/var/www/html/data`.

При первом запуске в этой директории должен находиться файл `.ocdata` со следующим содержимым:
```
# Nextcloud data directory
```

Скрипт `setup.sh` автоматически создает этот файл при первом запуске.

### База данных

Данные базы данных PostgreSQL хранятся в Docker volume и не затрагиваются при работе с файлами пользователей.